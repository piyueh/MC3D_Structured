...
!======================================================================
    ...
    IMPLICIT NONE
    REAL(KIND=8):: phm(iNprop), vel(3), sinratio
    REAL(KIND=8):: dsinth1, dcosth1, dsinth2, dcosth2, tmp
    INTEGER(KIND=4):: idx
    !------------------------------------------------------------------
...
!======================================================================
    ...
    IMPLICIT NONE
    INTEGER(KIND=4):: face0, nc, idx, i
    REAL(KIND=8):: phm(iNprop), tmp, tmp2, tmp3
    REAL(KIND=8):: rannum(2)
    !------------------------------------------------------------------
    ...
    !------------------------------------------------------------------
        ...
        IF ( idx.eq.2 ) THEN
            ...
            IF ( DABS( phm(4) / tmp2 ).gt.1d0 ) &  ! in case tmp3 = 0
...
!======================================================================
    ...
    IMPLICIT NONE
    INTEGER(KIND=4):: i, j, k, tot, m
    REAL(KIND=8), ALLOCATABLE:: lcr(:, :), newp(:, :)
        ...
        DO k = 1, iNcell(3)
            DO j = 1, iNcell(2)
                DO i = 1, iNcell(1)
                    ...
                    dEcell(i, j, k) = (SUM( phn(6, bg:ed) ) + &
                                             dEdiff(i, j, k)) / dVolume

                    CALL Etable( iCmat(i, j, k), 2, dEcell(i, j, k), &
                                                      dEunit(i, j, k) )
                    CALL Etable( iCmat(i, j, k), 4, dEcell(i, j, k), &
                                                      dVunit(i, j, k) )
                    CALL Etable( iCmat(i, j, k), 5, dEcell(i, j, k), &
                                                         MFP(i, j, k) )
                    CALL Etable( iCmat(i, j, k), 1, dEcell(i, j, k), &
                                                       dTemp(i, j, k) )
                    dEunit(i, j, k) = dEcell(i, j, k) / &
                                      dEunit(i, j, k) * &
                                      bundle(iCmat(i, j, k))
...
!======================================================================
    ...
    IMPLICIT NONE
    INTEGER(KIND=4) :: nc
    REAL(KIND=8) :: T0, Eout, Ea, Eb, Ta, Tb, Tout
